...
class Offering(models.Model):
    ...
    class Meta:
        ...
        managed = False  # Add this to prevent Django from modifying the table

    def __str__(self):
        return f"{self.offering_type} - {self.amount} - {self.payment_date}"

    def save(self, *args, **kwargs):
        if not self.receipt_no:  # Changed from receipt_number to receipt_no
            import random
            self.receipt_no = f"RCP-{self.church.id}-{random.randint(10000, 99999)}"  # Changed to receipt_no
