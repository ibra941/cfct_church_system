...
class AdminUserCreationForm(UserCreationForm):
    ...
    role = forms.ChoiceField(choices=ADD_ROLE_CHOICES)
    church = forms.ModelChoiceField(
        queryset=Church.objects.none(),
        required=False,
        help_text='Select the assignment level based on the selected role.',
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        selected_role = self.data.get('role') or self.initial.get('role')
        self.fields['church'].queryset = self._get_church_queryset_for_role(selected_role)

    def _get_church_queryset_for_role(self, role):
        if role == 'zone_leader':
            return Church.objects.filter(church_type='zone').order_by('name')
        if role == 'district_leader':
            return Church.objects.filter(church_type='district').order_by('name')
        if role == 'local_leader':
            return Church.objects.filter(church_type='local').order_by('name')
    ...
    def clean(self):
        ...
        if expected_type and church is None:
            raise forms.ValidationError('Please select the correct assignment before saving this leader.')

        if church and expected_type and church.church_type != expected_type:
            raise forms.ValidationError('Selected assignment does not match the selected role.')
