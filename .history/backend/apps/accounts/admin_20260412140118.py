...
class AdminUserCreationForm(UserCreationForm):
    ...
    def clean(self):
        ...
        if not church:
            self.add_error('church', f"Please select a {expected_type} for this role.")
            return cleaned_data

        if church.church_type != expected_type:
            self.add_error(
                'church',
                f"Selected church must be a {expected_type} when role is {self.fields['role'].choices_dict.get(role, role) if hasattr(self.fields['role'], 'choices_dict') else role}.",
            )
