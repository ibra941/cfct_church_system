        ...
        if user_account.email:
            try:
                send_email_notification(
                    'CFCT Registration Approved',
                    (
                        f'Hello {user_account.get_full_name()},\n\n'
                        'Your registration has been approved. You can now log in to the CFCT system.\n\n'
    ...
    def post(self, request, pk):
        ...
        try:
            create_notification(
                ...
                # Derive username from member's full name
                full_name = registration.personal_info.get('full_name', '') or registration.user.full_name or ''
                if full_name:
                    base_username = re.sub(r'[^a-z0-9_]', '', full_name.lower().replace(' ', '_'))
                    base_username = base_username[:30] if base_username else 'member'
                else:
                    base_username = 'member'
                new_username = base_username
                counter = 1
                while User.objects.filter(username=new_username).exclude(id=registration.user_id).exists():
                ...
                try:
                    create_notification(
                        ...
                        'Registration Approved',
                        (
                            f'Your registration has been approved by {user.full_name}. '
                            f'You can now access your church and log in with username: {new_username}'
                        ),
                        'success'
                    )
                except Exception as notify_error:
                ...
                if user_account.email:
                    try:
                        send_email_notification(
                            'CFCT Registration Approved - Your Login Credentials',
                            (
                                f'Dear {user_account.full_name or user_account.get_full_name()},\n\n'
                                f'Congratulations! You have been approved. You can now access your church '
                                f'({registration.church.name}) and be involved in all activities concerned with your church '
                                f'through this password.\n\n'
                                f'Your login credentials are:\n'
                                f'  Username: {new_username}\n'
                                f'  Password: {generated_password}\n\n'
                                f'Please log in and change your password after your first login for security.\n\n'
                                f'Blessings,\nCFCT Management Team'
                            ),
                            [user_account.email],
                        )
    ...
    def post(self, request):
        ...
        class ApproveRegistrationView(APIView):
            ...
            def post(self, request, pk):
                ...
                # Generate a secure random password for the approved member
                alphabet = string.ascii_letters + string.digits + "!@#$%^&*"
                generated_password = ''.join(secrets.choice(alphabet) for _ in range(12))
                ...
                uname_counter = 1
                while User.objects.filter(username=new_username).exclude(id=registration.user_id).exists():
                    new_username = f"{base_username}{uname_counter}"
                    uname_counter += 1
                    ...
                    try:
                        send_email_notification(
                            ...
                            (
                                f'Dear {user_account.full_name or user_account.get_full_name()},\n\n'
                                f'Congratulations! You have been approved. You can now access your church '
                                f'({registration.church.name}) and be involved in all activities concerned '
                                f'with your church through this password.\n\n'
                                f'Your login credentials are:\n'
                                f'  Username: {new_username}\n'
                                f'  Password: {generated_password}\n\n'
                                f'Please log in and change your password after your first login for security.\n\n'
        ...
        class RejectRegistrationView(APIView):
            ...
            def post(self, request, pk):
                ...
                if user_account.email:
                    try:
                        send_email_notification(
                            'CFCT Registration Update - Application Not Approved',
                            (
                                f'Dear {user_account.full_name or user_account.get_full_name()},\n\n'
                                f'We regret to inform you that your registration application for '
                                f'{registration.church.name} has not been approved at this time.\n\n'
                                f'Reason: {rejection_reason if rejection_reason else "No reason provided"}\n\n'
                                f'Please contact your church leadership for further assistance.\n\n'
                                f'Blessings,\nCFCT Management Team'
                            ),
                            [user_account.email],
                        )
