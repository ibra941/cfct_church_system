...
class LeaderRegisterMemberView(APIView):
    """
    Authenticated leaders register a member directly (auto-approved).
    Accepts the same payload structure as the public registration form.
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        ...
        if not preferred_church_id:
            return Response({'error': 'Please select a church'}, status=status.HTTP_400_BAD_REQUEST)

        # Validate church is in the leader's accessible scope
        try:
            church = get_accessible_churches(user).get(id=preferred_church_id)
        except Church.DoesNotExist:
            return Response(
                {'error': 'Selected church is not within your accessible scope'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # Use phone-based username if no email provided
        if email:
            ...
            username_base = f"member_{phone}" if phone else f"member_{int(timezone.now().timestamp())}"
        ...
        # Generate a random password; the member can reset it later
        ...
        with transaction.atomic():
            try:
                ...
                return Response({'error': f'Failed to create member: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            create_notification(
                user,
                'Member Registered',
                f'Member {new_user.full_name} has been registered at {church.name}',
                'success'
            )
        except Exception:
            pass
        ...
        return Response(
            {'message': 'Member registered successfully', 'user_id': new_user.id, 'username': username},
            status=status.HTTP_201_CREATED
        )
