...
def _national_report_payload(parameters):
    """Return national-level report payload with per-zone breakdown."""
    now = timezone.now()
    zones = Church.objects.filter(church_type='zone').order_by('name')
    rows = []
    totals = {'zones': zones.count(), 'regions': 0, 'districts': 0, 'locals': 0, 'members': 0, 'offerings': 0.0, 'events': 0}
    ...
    for zone in zones:
        ...
        members = User.objects.filter(church__in=zone_scope, role__in=['local_member', 'local_leader'], is_active=True).count()
        offerings = _safe_amount(Offering.objects.filter(church__in=zone_scope).aggregate(total=Sum('amount'))['total'])
        events = Event.objects.filter(church__in=zone_scope, is_active=True).filter(
            Q(end_date__gte=now) | Q(end_date__isnull=True, start_date__gte=now)
        ).count()
        ...
        rows.append(row)
        totals['regions'] += regions_count
        totals['districts'] += districts_count
        totals['locals'] += locals_count
        totals['members'] += members
        totals['offerings'] += offerings
        totals['events'] += events
...
def export_to_excel(report_type, parameters):
    ...
    if report_type == 'district':
        ...
        payload = _national_report_payload(parameters)
        sheet.append(['National Report'])
        sheet.append(['Generated At', payload['generated_at'].strftime('%Y-%m-%d %H:%M:%S')])
        if payload['notes']:
            sheet.append(['Notes', payload['notes']])
        sheet.append([])
        sheet.append(['Zone', 'Zone Leader', 'Leader Email', 'Regions', 'Districts', 'Local Churches', 'Members', 'Offerings', 'Events'])
        for row in payload['zones']:
            sheet.append([
                row['zone_name'],
                row['zone_leader_name'],
                row['zone_leader_email'],
                row['regions'],
                row['districts'],
                row['locals'],
                row['members'],
        ...
        sheet.append([])
        sheet.append(['Total Zones', payload['totals']['zones']])
        sheet.append(['Total Regions', payload['totals']['regions']])
        sheet.append(['Total Districts', payload['totals']['districts']])
        sheet.append(['Total Local Churches', payload['totals']['locals']])
        sheet.append(['Total Members', payload['totals']['members']])
...
def export_to_csv(report_type, parameters):
    ...
    if report_type == 'district':
        ...
        payload = _national_report_payload(parameters)
        writer.writerow(['National Report'])
        writer.writerow(['Generated At', payload['generated_at'].strftime('%Y-%m-%d %H:%M:%S')])
        if payload['notes']:
            writer.writerow(['Notes', payload['notes']])
        writer.writerow([])
        writer.writerow(['Zone', 'Zone Leader', 'Leader Email', 'Regions', 'Districts', 'Local Churches', 'Members', 'Offerings', 'Events'])
        for row in payload['zones']:
            writer.writerow([
                row['zone_name'],
                row['zone_leader_name'],
        ...
        writer.writerow([])
        writer.writerow(['Total Zones', payload['totals']['zones']])
        writer.writerow(['Total Regions', payload['totals']['regions']])
        writer.writerow(['Total Districts', payload['totals']['districts']])
        writer.writerow(['Total Local Churches', payload['totals']['locals']])
        writer.writerow(['Total Members', payload['totals']['members']])
...
def export_to_pdf(report_type, parameters):
    ...
    if report_type == 'district':
        ...
        payload = _national_report_payload(parameters)
        elements.append(Paragraph(f"Generated: {payload['generated_at'].strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
        data = [['Zone', 'Zone Leader', 'Regions', 'Districts', 'Locals', 'Members', 'Offerings', 'Events']]
        for row in payload['zones']:
            data.append([
                row['zone_name'],
                row['zone_leader_name'] or '-',
                str(row['regions']),
                str(row['districts']),
                str(row['locals']),
                str(row['members']),
