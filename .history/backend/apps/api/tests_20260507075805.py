...
class CoreApiTests(APITestCase):
    ...
    def test_live_health_endpoint_is_public(self):
        response = self.client.get('/api/health/live/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get('status'), 'ok')

    def test_ready_health_endpoint_is_public_when_db_ok(self):
        response = self.client.get('/api/health/ready/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get('status'), 'ready')

    def test_ready_health_endpoint_returns_503_when_db_unreachable(self):
        with patch('apps.api.views.connection.cursor', side_effect=Exception('db down')):
            response = self.client.get('/api/health/ready/')

        self.assertEqual(response.status_code, status.HTTP_503_SERVICE_UNAVAILABLE)
        self.assertEqual(response.data.get('status'), 'not_ready')
    ...
    def test_payment_callback_accepts_valid_signature(self):
        mock_offering = Mock()
        mock_offering.id = 101
        mock_offering.payment_status = 'pending'
        mock_offering.transaction_reference = 'MM-UNITTEST123'
        ...
        with patch('apps.api.views.Offering.objects.get', return_value=mock_offering):
            response = self.client.post(
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(mock_offering.payment_status, 'completed')
        mock_offering.save.assert_called_once()
    ...
    def test_payment_callback_rejects_invalid_signature(self):
        ...
        with patch('apps.api.views.Offering.objects.get') as mocked_get:
            response = self.client.post(
                '/api/offerings/payments/callback/',
                data=payload,
                format='json',
