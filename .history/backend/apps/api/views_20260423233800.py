...
class MemberDashboardView(APIView):
    ...
    def get(self, request):
        ...
        member_events = Event.objects.filter(
            ...
            Q(created_by__role__in=GLOBAL_BROADCAST_ROLES)
            | Q(church=church, created_by__role='local_leader')
        ...
        for row in offering_totals:
            offering_summary[row['offering_type']] = float(row['total'] or 0)
        ...
        for prayer in PrayerRequest.objects.filter(member=user).order_by('-created_at')[:50]:
            activities.append(
                {
                    'type': 'prayer',
        ...
        for registration in EventRegistration.objects.filter(member=user).select_related('event').order_by('-registered_at')[:50]:
            activities.append(
                {
                    'type': 'event',
        ...
        for join_request in DepartmentJoinRequest.objects.filter(member=user).select_related('department').order_by('-requested_at')[:50]:
            activities.append(
                {
                    'type': 'department',
        ...
        activities = sorted(activities, key=lambda item: item['created_at'], reverse=True)[:200]
