...
MEMBER_USER_ROLES = ['local_member', 'local_leader']
...
def _query_param_value(request, key):
    return (request.query_params.get(key) or '').strip()


def _has_hierarchy_filters(request):
    return any(
        _query_param_value(request, key)
        for key in ['zone_id', 'region_id', 'district_id', 'church_name']
    )


def get_filtered_local_churches(user, request):
    queryset = get_accessible_churches(user).filter(church_type='local')
    zone_id = _query_param_value(request, 'zone_id')
    region_id = _query_param_value(request, 'region_id')
    district_id = _query_param_value(request, 'district_id')
    church_name = _query_param_value(request, 'church_name')

    if zone_id:
        queryset = queryset.filter(parent_church__parent_church__parent_church_id=zone_id)
    if region_id:
        queryset = queryset.filter(parent_church__parent_church_id=region_id)
    if district_id:
        queryset = queryset.filter(parent_church_id=district_id)
    if church_name:
        queryset = queryset.filter(name__icontains=church_name)
...
class ChurchViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        church_type = _query_param_value(self.request, 'church_type')
        ...
        elif _has_hierarchy_filters(self.request):
            queryset = queryset.filter(church_type='local')

        zone_id = _query_param_value(self.request, 'zone_id')
        region_id = _query_param_value(self.request, 'region_id')
        district_id = _query_param_value(self.request, 'district_id')
...
class MemberViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'national_leader':
            ...
            queryset = User.objects.filter(role__in=MEMBER_USER_ROLES)
            ...
            queryset = User.objects.filter(church__in=zone_churches, role__in=MEMBER_USER_ROLES)
            ...
            queryset = User.objects.filter(church__in=region_churches, role__in=MEMBER_USER_ROLES)
            ...
            queryset = User.objects.filter(church__in=district_churches, role__in=MEMBER_USER_ROLES)
        
        elif user_church:
            # Local leader or member sees only their church
            queryset = User.objects.filter(church=user_church, role__in=MEMBER_USER_ROLES)

        queryset = queryset.filter(church__in=get_filtered_local_churches(user, self.request))
...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not user.is_authenticated:
            ...
            if _has_hierarchy_filters(self.request):
                queryset = queryset.filter(church__in=get_filtered_local_churches(user, self.request))
...
class UpdateCurrentUserView(APIView):
    ...
    def patch(self, request):
        user = request.user
        allowed_fields = {'full_name', 'email', 'phone', 'neighborhood'}
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        def members_queryset_for(churches_queryset=None):
            base = User.objects.filter(role__in=MEMBER_USER_ROLES)
            if churches_queryset is not None:
                base = base.filter(church__in=churches_queryset)
            return base

        def member_registrations_queryset_for(churches_queryset=None):
            base = MemberRegistration.objects.all()
            if churches_queryset is not None:
                base = base.filter(church__in=churches_queryset)
            return base
        ...
        if context_church_id and user.role in ['national_leader', 'zone_leader', 'regional_leader', 'district_leader']:
            try:
                ...
                ct = ctx.church_type
                if ct == 'zone':
                    local_scope = Church.objects.filter(
                        church_type='local',
                        parent_church__parent_church__parent_church=ctx,
                    )
                    stats['total_churches'] = local_scope.count()
                    stats['total_members'] = (
                        members_queryset_for(local_scope).count()
                        + member_registrations_queryset_for(local_scope).count()
                    )
                    ...
                    stats['locals'] = local_scope.count()
                    stats['pending_approvals'] = pending_count_for(local_scope)
                    apply_scope_metrics(local_scope)
                    recent_members = members_queryset_for(local_scope).order_by('-created_at')
                elif ct == 'region':
                    local_scope = Church.objects.filter(
                        church_type='local',
                        parent_church__parent_church=ctx,
                    )
                    stats['total_churches'] = local_scope.count()
                    stats['total_members'] = (
                        members_queryset_for(local_scope).count()
                    ...
                    local_scope = Church.objects.filter(church_type='local', parent_church=ctx)
                    stats['total_churches'] = local_scope.count()
                    stats['total_members'] = (
                        members_queryset_for(local_scope).count()
                        + member_registrations_queryset_for(local_scope).count()
                    ...
                    stats['total_members'] = (
                        members_queryset_for(local_qs).count()
                        + member_registrations_queryset_for(local_qs).count()
                    )
        ...
        if user.role == 'national_leader':
            # National leader sees admin-aligned totals based on local churches.
            local_churches = get_filtered_local_churches(user, request)
            stats['total_members'] = (
                members_queryset_for(local_churches).count()
                + member_registrations_queryset_for(local_churches).count()
            )
            stats['total_churches'] = local_churches.count()
            stats['zones'] = Church.objects.filter(
                church_type='zone',
                id__in=local_churches.values_list('parent_church__parent_church__parent_church_id', flat=True),
            ).distinct().count()
            stats['regions'] = Church.objects.filter(
                church_type='region',
                id__in=local_churches.values_list('parent_church__parent_church_id', flat=True),
            ).distinct().count()
            stats['districts'] = Church.objects.filter(
                church_type='district',
                id__in=local_churches.values_list('parent_church_id', flat=True),
            ).distinct().count()
            stats['locals'] = stats['total_churches']
            stats['pending_approvals'] = pending_count_for(local_churches)
            apply_scope_metrics(local_churches)
            
            # Get recent members
            recent_members = members_queryset_for(local_churches).order_by('-created_at')
