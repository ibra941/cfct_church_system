...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return User.objects.filter(church__in=accessible_churches)
...
class TransferViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        elif user.role in ['district_leader', 'regional_leader', 'zone_leader'] and transfer.approved_by_from and transfer.approved_by_to:
            # Higher-level approval already covers both sides.
            pass
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def broadcast(self, request):
        ...
        valid_roles = ['local_leader', 'local_member', 'district_leader', 'regional_leader', 'all']
...
class ChurchComparisonView(APIView):
    ...
    def get(self, request):
        ...
        accessible_churches = get_accessible_churches(user)
        # Filter to local churches for meaningful per-church comparison
        local_churches = accessible_churches.filter(church_type='local')

        # Optional district filter
        district_id = request.query_params.get('district_id')
        if district_id:
            local_churches = local_churches.filter(
                Q(id=district_id) | Q(parent_church_id=district_id)
            ).filter(church_type='local')

        now = timezone.now()
        comparison_data = []
        for church in local_churches.order_by('name'):
