...
def _totp_code(secret: str, for_time: float | None = None, period: int = 30, digits: int = 6) -> str:
...
def _verify_totp(secret: str, code: str, period: int = 30, digits: int = 6, drift_steps: int = 1) -> bool:
    ...
    for step in range(-drift_steps, drift_steps + 1):
        if _totp_code(secret, for_time=now + (step * period), period=period, digits=digits) == candidate:
            return True
    return False


def _issue_2fa_challenge(user: User) -> str:
    payload = {
        'uid': user.id,
        'username': user.username,
        'nonce': secrets.token_hex(8),
    }
    return signing.dumps(payload, salt='cfct-2fa-login')


def _validate_2fa_challenge(challenge: str, user: User, max_age_seconds: int = 300) -> bool:
    try:
        payload = signing.loads(challenge, salt='cfct-2fa-login', max_age=max_age_seconds)
    except signing.BadSignature:
        return False
    return str(payload.get('uid')) == str(user.id) and payload.get('username') == user.username


def _extract_webhook_signature(request) -> str:
    header_name = getattr(settings, 'PAYMENT_WEBHOOK_SIGNATURE_HEADER', 'X-Azampay-Signature')
    normalized = header_name.upper().replace('-', '_')
    return (request.META.get(f'HTTP_{normalized}') or '').strip()


def _is_valid_webhook_signature(request) -> tuple[bool, str]:
    secret = (getattr(settings, 'PAYMENT_WEBHOOK_SIGNATURE_SECRET', '') or '').strip()
    require_signature = bool(getattr(settings, 'PAYMENT_WEBHOOK_REQUIRE_SIGNATURE', False))

    # When no secret is configured, allow callbacks only if strict verification is disabled.
    if not secret:
        if require_signature:
            return False, 'Webhook signature secret is not configured.'
        return True, ''

    provided_signature = _extract_webhook_signature(request)
    if not provided_signature:
        return False, 'Webhook signature header is missing.'
...
class LoginTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        otp_code = (request.data.get('otp_code', '') or '').strip() if hasattr(request, 'data') else ''
        two_factor_challenge = (request.data.get('two_factor_challenge', '') or '').strip() if hasattr(request, 'data') else ''
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            if (
                user
                and user.role in HIGH_PRIVILEGE_2FA_ROLES
                and user.two_factor_enabled
                and user.two_factor_secret
            ):
                if not otp_code or not two_factor_challenge:
                    return Response(
                        {
                            'requires_2fa': True,
                            'detail': 'Two-factor authentication code required.',
                            'two_factor_challenge': _issue_2fa_challenge(user),
                        },
                        status=status.HTTP_202_ACCEPTED,
                    )

                if not _validate_2fa_challenge(two_factor_challenge, user):
                    return Response(
                        {'detail': 'Invalid or expired two-factor challenge.'},
                        status=status.HTTP_401_UNAUTHORIZED,
                    )

                if not _verify_totp(user.two_factor_secret, otp_code):
                    return Response(
...
class TwoFactorSetupView(APIView):
    ...
    def post(self, request):
        user = request.user
        if user.role not in HIGH_PRIVILEGE_2FA_ROLES:
            return Response({'detail': '2FA setup is restricted to leadership accounts.'}, status=status.HTTP_403_FORBIDDEN)

        secret = user.two_factor_secret or _generate_totp_secret()
        user.two_factor_secret = secret
        user.save(update_fields=['two_factor_secret', 'updated_at'])

        issuer = 'CFCT Church System'
        account_name = user.email or user.username
        otpauth_url = (
            f"otpauth://totp/{_urlquote(issuer)}:{_urlquote(account_name)}"
            f"?secret={secret}&issuer={_urlquote(issuer)}&algorithm=SHA1&digits=6&period=30"
        )

        return Response({
            'detail': '2FA secret generated. Confirm with a valid authenticator code to activate.',
            'manual_key': secret,
...
class TwoFactorConfirmView(APIView):
    ...
    def post(self, request):
        ...
        if not user.two_factor_secret:
            return Response({'detail': '2FA is not initialized. Run setup first.'}, status=status.HTTP_400_BAD_REQUEST)

        if not _verify_totp(user.two_factor_secret, code):
        ...
        return Response({'detail': 'Two-factor authentication has been enabled successfully.'})
...
class TwoFactorDisableView(APIView):
    ...
    def post(self, request):
        ...
        user.save(update_fields=['two_factor_enabled', 'two_factor_secret', 'two_factor_confirmed_at', 'updated_at'])
...
class OfferingViewSet(viewsets.ModelViewSet):
    ...
    def my_history(self, request):
        # Leaders see all offerings for their accessible churches.
        # Members see only their own personal offerings.
        if request.user.role in LEADER_ROLES:
    ...
    def perform_create(self, serializer):
        ...
        # When a local_member records their own offering without specifying a member,
        # auto-link the offering to themselves so it appears in their records.
        if member is None and self.request.user.role == 'local_member':
...
class UpdateCurrentUserView(APIView):
    ...
    def patch(self, request):
        ...
        if 'profile_picture' in payload:
            ...
            if hasattr(pic, 'content_type') and pic.content_type not in allowed_content_types:
                return Response({'detail': 'Invalid image type. Allowed: JPEG, PNG, WebP, GIF.'}, status=status.HTTP_400_BAD_REQUEST)
            if hasattr(pic, 'size') and pic.size > max_size:
                return Response({'detail': 'Image file too large. Maximum size is 5 MB.'}, status=status.HTTP_400_BAD_REQUEST)
...
class LiveHealthCheckView(APIView):
    """Lightweight liveness endpoint for uptime monitors and orchestrators."""

    permission_classes = [permissions.AllowAny]
    authentication_classes = []

    def get(self, request):
        return Response({'status': 'ok', 'service': 'cfct-backend'})


class ReadyHealthCheckView(APIView):
    """Readiness endpoint that validates DB connectivity."""
    ...
    def get(self, request):
        ...
        return Response({'status': 'ready', 'dependencies': {'database': 'ok'}})
...
class ChurchTopView(APIView):
    """
    Return top N churches ranked by total offerings.

    Query params:
      limit         (int)    – how many churches to return (default 5)
      church_type   (str)    – filter by church type, e.g. 'local' (optional)
      region_id     (int)    – filter to a specific region's subtree (optional)
      district_id   (int)    – filter to a specific district's subtree (optional)
    """
    permission_classes = [permissions.IsAuthenticated]
    authentication_classes = [JWTAuthentication]
    ...
    def get(self, request):
        ...
        church_type = (request.query_params.get('church_type') or '').strip()
        region_id = request.query_params.get('region_id')
        district_id = request.query_params.get('district_id')

        queryset = get_accessible_churches(request.user)
        ...
        queryset = (
            queryset
            .annotate(
                total_offerings=Coalesce(Sum('offerings__amount'), Value(0, output_field=DecimalField())),
                member_count=Count('church_users', distinct=True, filter=Q(church_users__is_active=True)),
            )
        ...
        data = [
            {
                ...
                'name': church.name,
                'church_type': church.church_type,
                'offerings': float(church.total_offerings),
                'members': church.member_count,
            }
...
class MobileMoneyPaymentView(APIView):
    ...
    def post(self, request):
        ...
        # When a local_member pays without specifying a member, link to themselves
        if member is None and user.role == 'local_member':
            member = user
...
class PaymentCallbackView(APIView):
    ...
    def post(self, request):
        signature_ok, signature_error = _is_valid_webhook_signature(request)
        if not signature_ok:
            return Response({'error': signature_error}, status=status.HTTP_401_UNAUTHORIZED)
