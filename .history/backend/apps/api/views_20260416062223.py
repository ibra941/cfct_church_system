...
class TransferViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        if transfer.from_church == user.church or user.role == 'national_leader':
...
class AuditLogsView(APIView):
    ...
    def get(self, request):
        ...
        data = [
            {
                'id': log.id,
                'username': log.user.get_full_name() if hasattr(log.user, 'get_full_name') else log.user.username,
