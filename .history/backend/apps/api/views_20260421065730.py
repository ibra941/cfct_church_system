...
class OfferingViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.church:
            ...
            raise ValidationError({"church": "Your profile must be assigned to a church to record offerings."})
        
        # Parse member if provided as string
        member = serializer.validated_data.get('member')
        if member == '' or member is None:
            member = None
        else:
            # If member is a string ID, try to convert it
