  ...
  const login = useCallback(
    async (username, password, options = {}) => {
      ...
      try {
        ...
        if (twoFactorChallenge) payload.two_factor_challenge = twoFactorChallenge;
        ...
        if (response.status === 202 && response.data?.requires_2fa) {
          return {
            success: false,
            requires2FA: true,
            challengeToken: response.data?.two_factor_challenge || "",
            detail: response.data?.detail || "Two-factor authentication code required.",
          };
        }
        ...
        if (statusCode === 401 || statusCode === 400) {
          const lowerDetail = String(apiDetail || "").toLowerCase();
          if (
            lowerDetail.includes("authentication code") ||
            lowerDetail.includes("two-factor") ||
            lowerDetail.includes("challenge")
          ) {
            toast.error(apiDetail || "Two-factor authentication failed.");
          } else {
