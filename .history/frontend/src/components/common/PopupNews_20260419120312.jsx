...
const PopupNews = ({ onClose }) => {
  ...
  const fetchPopupNews = async () => {
    try {
      ...
      const popupItems = extractListData(response.data);
      ...
      try {
        ...
        const popupItems = extractListData(fallbackResponse.data);
        if (popupItems.length > 0) {
          setPopups(popupItems);
        }
      } catch (fallbackError) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 animate-fade-in p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full overflow-hidden relative">
        ...
        {/* Image */}
        {(currentPopup.primary_image || currentPopup.images?.[0]) && (
          <img
            src={currentPopup.primary_image || currentPopup.images?.[0]}
