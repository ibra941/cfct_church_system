...
import { FaSun, FaMoon, FaGlobe, FaUser, FaSignOutAlt, FaBars } from 'react-icons/fa'
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              <Link to="/" className="flex-shrink-0 flex items-center">
                ...
                <span className="ml-2 text-xl font-bold text-primary-600 dark:text-primary-400">CFCT</span>
              </Link>
            </div>
              ...
              {isAuthenticated ? (
                <div className="relative group">
                  <button className="flex items-center space-x-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <span className="text-gray-700 dark:text-gray-200">{user.full_name || user.username}</span>
                  </button>
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg hidden group-hover:block">
                    <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                ...
                <Link to="/login" className="btn-primary">{language === 'sw' ? 'Ingia' : 'Login'}</Link>
              )}
            </div>
          </div>
        </div>
