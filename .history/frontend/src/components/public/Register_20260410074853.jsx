...
const Register = () => {
  ...
  const fetchChurchHierarchyData = async () => {
    ...
    try {
      const [zonesResponse, regionsResponse, districtsResponse, localsResponse] =
        await Promise.all([
          api.get("/zones/"),
          api.get("/regions/"),
          api.get("/districts/"),
  ...
  const getParentId = (item) =>
    item?.parent_church?.id ?? item?.parent_church ?? item?.parent_church_id;

  const filteredRegions = regions.filter(
    (region) => String(getParentId(region)) === String(selectedZone),
  );

  const filteredDistricts = districts.filter(
    (district) => String(getParentId(district)) === String(selectedRegion),
  );

  const filteredLocalChurches = localChurches.filter(
    (local) => String(getParentId(local)) === String(selectedDistrict),
  );
              ...
              <div className="space-y-4">
                ...
                {selectedZone && (
                  <div>
                    ...
                    {selectedZone && filteredRegions.length === 0 && (
                ...
                {selectedDistrict && (
                  <div>
                    ...
                    >
                      ...
                      {filteredLocalChurches.map((church) => (
                        <option key={church.id} value={church.id}>
                          {church.name}
                        </option>
                      ))}
                    </select>
                    {selectedDistrict && filteredLocalChurches.length === 0 && (
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                        {language === "sw"
