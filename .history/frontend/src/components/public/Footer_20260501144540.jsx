...
const Footer = () => {
  ...
  const copyrightText =
    siteSettingsMap.copyright_text || `© ${new Date().getFullYear()} CFCT. All rights reserved.`;
              ...
              {quickLinks.map((item, index) => (
                <li key={`${item.title}-${index}`}>
                  {item.is_external ? (
                    ...
                    <Link to={item.url} className="text-gray-400 hover:text-white">
                      {item.title}
                    </Link>
                  )}
                </li>
          ...
          <div>
            ...
            <ul className="space-y-2">
              <li className="flex items-center space-x-2">
                ...
                <span className="text-gray-400">{contactMap.email || "cfctanzaniahq2019@gmail.com"}</span>
              </li>
              <li className="flex items-center space-x-2">
                <FaPhone className="text-gray-400" />
                <span className="text-gray-400">{contactMap.phone || "+255 767 911 316"}</span>
              </li>
            </ul>
          </div>
          <div>
