      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        {backDashboardPath && (
          ...
          >
            ...
            {language === "sw" ? "Rudi Dashibodi Kuu" : "Back to Leader Dashboard"}
          </Link>
        )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-2">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <span className="font-semibold">{language === "sw" ? "Kanisa" : "Church"}:</span>{" "}
            {data?.church?.name || "-"}
          </p>
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <span className="font-semibold">{language === "sw" ? "Mchungaji" : "Pastor"}:</span>{" "}
            {data?.pastor?.name || "-"}
          </p>
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <span className="font-semibold">Email:</span> {data?.pastor?.email || "-"}
          </p>
          <p className="text-sm text-gray-700 dark:text-gray-300 flex items-center">
            <FaPhone className="mr-2" />
            <span className="font-semibold mr-1">{language === "sw" ? "Simu" : "Phone"}:</span>
            {data?.pastor?.phone || "-"}
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          ...
          <form onSubmit={updateContact} className="space-y-3 mb-6">
            ...
            />
            <button className="btn-secondary w-full" type="submit" disabled={savingContact}>
              {savingContact
                ? language === "sw"
                  ? "Inahifadhi..."
                  : "Saving..."
          ...
          <form onSubmit={changePassword} className="space-y-3">
            <input
              type="password"
              className="input"
              placeholder={language === "sw" ? "Nenosiri la Sasa" : "Current Password"}
            ...
            <input
              type="password"
              className="input"
              placeholder={language === "sw" ? "Nenosiri Jipya" : "New Password"}
            ...
            <input
              type="password"
              className="input"
              placeholder={language === "sw" ? "Thibitisha Nenosiri Jipya" : "Confirm New Password"}
              value={passwordForm.confirm_password}
              onChange={(e) => setPasswordForm((prev) => ({ ...prev, confirm_password: e.target.value }))}
            />
            <button className="btn-primary w-full" type="submit" disabled={changingPassword}>
              {changingPassword
                ? language === "sw"
                  ? "Inabadilisha..."
                  : "Changing..."
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          ...
          <form onSubmit={submitOffering} className="space-y-3 mb-5">
            <select
              ...
              onChange={(e) => setOfferingForm((prev) => ({ ...prev, offering_type: e.target.value }))}
            >
              <option value="tithe">{language === "sw" ? "Zaka" : "Tithe"}</option>
              <option value="offering">{language === "sw" ? "Sadaka" : "Offering"}</option>
              <option value="building">{language === "sw" ? "Jengo" : "Building"}</option>
              <option value="mission">{language === "sw" ? "Misheni" : "Mission"}</option>
            </select>
            <input
              type="number"
              step="0.01"
            ...
            <select
              ...
              onChange={(e) => setOfferingForm((prev) => ({ ...prev, payment_method: e.target.value }))}
            >
              <option value="cash">{language === "sw" ? "Taslimu" : "Cash"}</option>
            ...
            <textarea
              rows="2"
              className="input"
              placeholder={language === "sw" ? "Maelezo (hiari)" : "Notes (optional)"}
              value={offeringForm.notes}
              onChange={(e) => setOfferingForm((prev) => ({ ...prev, notes: e.target.value }))}
            />
            <button className="btn-primary w-full" type="submit" disabled={submittingOffering}>
              {submittingOffering
                ? language === "sw"
                  ? "Inawasilisha..."
                  : "Submitting..."
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-gray-100 dark:bg-gray-700 p-3 text-sm">
              ...
              <p className="font-semibold text-gray-900 dark:text-white">{offeringSummary.tithe || 0}</p>
            </div>
            <div className="rounded-lg bg-gray-100 dark:bg-gray-700 p-3 text-sm">
              <p className="text-gray-500 dark:text-gray-300">Offering</p>
              <p className="font-semibold text-gray-900 dark:text-white">{offeringSummary.offering || 0}</p>
            </div>
            <div className="rounded-lg bg-gray-100 dark:bg-gray-700 p-3 text-sm">
              <p className="text-gray-500 dark:text-gray-300">Building</p>
              <p className="font-semibold text-gray-900 dark:text-white">{offeringSummary.building || 0}</p>
            </div>
            <div className="rounded-lg bg-gray-100 dark:bg-gray-700 p-3 text-sm">
              <p className="text-gray-500 dark:text-gray-300">Mission</p>
              <p className="font-semibold text-gray-900 dark:text-white">{offeringSummary.mission || 0}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto">
          ...
          {activities.length === 0 && (
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "sw" ? "Hakuna rekodi bado" : "No activity records yet"}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto">
          ...
          {(data?.offerings || []).length === 0 && (
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "sw" ? "Hakuna matoleo yaliyorekodiwa" : "No offerings recorded yet"}
            </p>
          )}
        </div>
      </div>
