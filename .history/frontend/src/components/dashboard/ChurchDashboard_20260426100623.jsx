  ...
  const fetchChurchData = async () => {
    ...
    try {
      ...
      if (user?.role === "local_leader") {
        ...
        const prayerRequestsResponse = await api.get("/prayers/?status=pending");
        ...
        <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          <div className="space-y-4">
            {pendingPrayerRequests.map((request) => (
              ...
              >
                <p className="font-semibold text-gray-900 dark:text-white">
                  {request.member_name || language === "sw" ? "Mwanachama" : "Member"}
                </p>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  >
                    <FaCheck />
                    <span>{language === "sw" ? "Weka Imekamilika" : "Mark Done"}</span>
                  </button>
                  <button
                    type="button"
