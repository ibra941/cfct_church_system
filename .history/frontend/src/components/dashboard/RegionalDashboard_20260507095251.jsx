...
const RegionalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch top churches by offerings — only local churches within this region
      try {
        const topParams = new URLSearchParams({ limit: "5", church_type: "local" });
        if (regionId) topParams.set("region_id", regionId);
        const topChurchesResponse = await api.get(`/churches/top/?${topParams}`);
        const topChurchesData = Array.isArray(topChurchesResponse.data)
          ? topChurchesResponse.data
          : extractListData(topChurchesResponse.data);
      ...
      // Fetch district leader candidates — include local_leaders (pastors) since
      // a pastor can be appointed as a district leader.
      try {
        const [distLeadersRes, localLeadersRes] = await Promise.all([
          api.get("/users/?role=district_leader"),
          api.get("/users/?role=local_leader"),
        ]);
        const merged = [
          ...extractListData(distLeadersRes.data),
          ...extractListData(localLeadersRes.data),
        ];
