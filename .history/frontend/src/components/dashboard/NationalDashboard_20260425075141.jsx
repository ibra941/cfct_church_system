...
const NationalDashboard = () => {
  ...
  useEffect(() => {
    fetchDashboardData();
  }, [filters.zoneId, filters.regionId, filters.districtId, filters.churchName]);
  ...
  const buildFilterParams = () => {
    ...
    if (filters.churchName.trim()) params.set("church_name", filters.churchName.trim());
    return params;
  };
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const membersResponse = await api.get(withParams("/members/", { limit: 5 }));
      const membersData = extractListData(membersResponse.data);
      setRecentMembers(Array.isArray(membersData) ? membersData : []);

      // Fetch upcoming events
      const eventsResponse = await api.get(withParams("/events/", { upcoming: true }));
    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          <label className="block">
            ...
            >
              <option value="">{language === "sw" ? "Kanda zote" : "All zones"}</option>
              {zones.map((zone) => (
          ...
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-white"
            >
              <option value="">{language === "sw" ? "Mikoa yote" : "All regions"}</option>
              {regions.map((region) => (
          ...
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-white"
            >
              <option value="">{language === "sw" ? "Wilaya zote" : "All districts"}</option>
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => handleFilterChange("churchName", event.target.value)}
              placeholder={language === "sw" ? "Tafuta kanisa" : "Search church name"}
              className="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-900 dark:text-white"
            />
          </label>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">
              {language === "sw" ? "Makanisa Yaliyopatikana" : "Matched Churches"}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "sw"
            ...
            {churches.slice(0, 9).map((church) => (
              ...
              >
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {church.city || (language === "sw" ? "Hakuna mji" : "No city")}
                </p>
              </div>
            ))}
          </div>
