    ...
    <div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: language === "sw" ? "Jumla" : "Total", value: transferStats.total, color: "text-gray-700 dark:text-gray-200" },
            { label: language === "sw" ? "Inasubiri" : "Pending", value: transferStats.pending, color: "text-yellow-600" },
            { label: language === "sw" ? "Zilizoidhinishwa" : "Approved", value: transferStats.approved, color: "text-green-600" },
            { label: language === "sw" ? "Zilizokataliwa" : "Rejected", value: transferStats.rejected, color: "text-red-600" },
          ].map((item, i) => (
        ...
        {transferStats.pending > 0 && (
          <div className="mt-3 text-right">
            <Link to="/transfers" className="text-sm text-primary-600 hover:text-primary-700">
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Ulinganisho wa Wilaya" : "District Performance Comparison"}
          </h2>
          <FaChartBar className="text-gray-400" />
        </div>
        {districtComparison.length === 0 ? (
          <p className="text-gray-500 text-center py-6">
            {language === "sw" ? "Hakuna data ya wilaya" : "No district data available"}
          </p>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700">
                  <th className="px-4 py-2 text-left text-gray-600 dark:text-gray-300">{language === "sw" ? "Wilaya" : "District"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Makanisa" : "Churches"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Wanachama" : "Members"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Wapya (Mwezi)" : "New (Month)"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Matoleo" : "Offerings"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Matukio" : "Events"}</th>
                  <th className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{language === "sw" ? "Maombi Yanayosubiri" : "Pending Regs"}</th>
                  <th className="px-4 py-2 text-left text-gray-600 dark:text-gray-300">{language === "sw" ? "Kiongozi" : "Leader"}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {districtComparison.map((d) => (
                  <tr key={d.district_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">
                      <button onClick={() => navigate(`/dashboard/district/${d.district_id}`)} className="hover:text-primary-600">
                        {d.district_name}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{d.local_churches_count}</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{d.members_count}</td>
                    <td className="px-4 py-3 text-right text-green-600">+{d.new_members_this_month}</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{formatCurrency(d.offerings_total)}</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{d.events_count}</td>
                    <td className="px-4 py-3 text-right">
                      {d.pending_registrations > 0 ? (
                        <span className="text-yellow-600 font-semibold">{d.pending_registrations}</span>
                      ) : (
                        <span className="text-gray-400">0</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-300">
                      {d.district_leader_name || <span className="text-gray-400 italic">{language === "sw" ? "Hajapatikana" : "Unassigned"}</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Muhtasari wa Fedha za Mkoa" : "Regional Financial Summary"}
          </h2>
          <FaMoneyBillWave className="text-gray-400" />
        </div>
        <div className="mb-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">{language === "sw" ? "Jumla ya Matoleo" : "Total Offerings"}</p>
          <p className="text-2xl font-bold text-green-600">{formatCurrency(regionalFinancial.grand_total)}</p>
        </div>
        {regionalFinancial.by_district.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">
              {language === "sw" ? "Matoleo kwa Wilaya" : "Offerings by District"}
            </h3>
            <div className="space-y-2">
              {regionalFinancial.by_district.map((d) => (
                <div key={d.district_id} className="flex items-center justify-between py-2 border-b dark:border-gray-700">
                  <span className="text-gray-900 dark:text-white text-sm">{d.district_name}</span>
                  <span className="text-green-600 font-medium text-sm">{formatCurrency(d.total_offerings)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {regionalFinancial.by_type.map((t) => (
                <div key={t.type} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                  <p className="text-sm font-medium text-gray-900 dark:text-white capitalize">{t.type}</p>
                  <p className="text-green-600 font-semibold mt-1">{formatCurrency(t.total)}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        {districtLeaders.length === 0 ? (
          <p className="text-gray-500 text-center py-4">
            {language === "sw" ? "Hakuna viongozi wa wilaya" : "No district leaders found"}
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {districtLeaders.map((leader) => (
              <div key={leader.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    {leader.phone && <p className="text-xs text-gray-500 dark:text-gray-400">{leader.phone}</p>}
                    {leader.church_name && (
                      <p className="text-xs text-primary-600 mt-1">{leader.church_name}</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="mt-4 text-right">
          ...
          >
            {language === "sw" ? "Teua Kiongozi wa Wilaya" : "Assign District Leader"}
          </button>
        </div>
      </div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Tuma Ujumbe kwa Viongozi" : "Broadcast to District Leaders"}
          </h2>
          ...
          <form onSubmit={handleBroadcastSend} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                <option value="local_leader">{language === "sw" ? "Wachungaji" : "Pastors"}</option>
            ...
            <div className="flex space-x-3">
              ...
              <button
                type="button"
                onClick={() => { setShowBroadcastPanel(false); setBroadcastMsg(""); }}
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Omba Rasilimali kutoka Kanda" : "Request Resources from Zone"}
          </h2>
          ...
          <form onSubmit={handleResourceRequest} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setResourceForm({ ...resourceForm, message: e.target.value })}
                required
                className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              />
            </div>
            ...
            <div className="flex space-x-3">
              ...
              <button
                type="button"
                onClick={() => { setShowResourceModal(false); setResourceMsg(""); }}
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition text-sm"
              >
                {language === "sw" ? "Ghairi" : "Cancel"}
              </button>
            ...
            <form onSubmit={handleAssignDistrictLeader} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setAssignTargetChurch(districtData.find((d) => String(d.id) === e.target.value) || null)}
                  required
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                >
                  <option value="">{language === "sw" ? "-- Chagua --" : "-- Select --"}</option>
                  {districtData.map((d) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {language === "sw" ? "ID ya Mtumiaji" : "User ID"}
                </label>
                <input
                  type="number"
                  value={assignUserId}
                  ...
                  required
                  placeholder={language === "sw" ? "Weka ID ya mtumiaji" : "Enter user ID"}
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                />
              </div>
              {assignMsg && (
              ...
              <div className="flex space-x-3">
                ...
                >
                  {assignLoading
                    ...
                    : (language === "sw" ? "Teua" : "Assign")}
                </button>
                <button
                  type="button"
                  onClick={() => { setShowAssignModal(false); setAssignMsg(""); }}
                  className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition text-sm"
                >
