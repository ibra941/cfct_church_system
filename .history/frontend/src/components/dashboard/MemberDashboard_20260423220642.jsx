...
const MemberDashboard = () => {
  ...
  const statusLabel = (status) => {
    if (status === "approved") return language === "sw" ? "Imekubaliwa" : "Approved";
    if (status === "pending") return language === "sw" ? "Inasubiri" : "Pending";
    if (status === "rejected") return language === "sw" ? "Imekataliwa" : "Rejected";
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {(data?.leaders || []).map((leader) => (
            <div key={leader.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <p className="font-semibold text-gray-900 dark:text-white">{leader.full_name}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">{leader.role}</p>
              <p className="text-sm text-gray-700 dark:text-gray-300 mt-2">{leader.email || "-"}</p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          ...
          <form onSubmit={submitPrayer} className="space-y-3">
            ...
            />
            <button type="submit" className="btn-primary w-full" disabled={submittingPrayer}>
              {submittingPrayer
                ? language === "sw"
                  ? "Inatuma..."
                  : "Submitting..."
          ...
          <div className="mt-4 space-y-2 max-h-48 overflow-y-auto">
            {(data?.prayers || []).map((prayer) => (
              <div key={prayer.id} className="text-sm border-b border-gray-100 dark:border-gray-700 pb-2">
                <p className="text-gray-800 dark:text-gray-200">{prayer.request}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(prayer.created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
          ...
          <div className="space-y-3">
            {(data?.departments || []).map((dept) => (
              <div key={dept.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white">{dept.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{dept.leader_name || "-"}</p>
                  </div>
                  <span className={`px-2 py-1 text-xs rounded-full ${statusClassName(dept.status)}`}>
          ...
          {upcomingEvents.map((event) => (
            <div key={event.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-400">{event.venue || "-"}</span>
                <button
