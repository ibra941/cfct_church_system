...
const NationalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch hierarchy items for search and view
  ...
  return (
    <div>
      {/* Search and hierarchy discovery for national leadership */}
      <div className="grid gap-4 mb-6 lg:grid-cols-3">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                {language === "sw" ? "Tafuta Kanda" : "Search Zones"}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw"
                  ? "Tafuta kanda kwa jina"
                  : "Search by zone name"}
              </p>
            </div>
          </div>
          <input
            ...
            placeholder={language === "sw" ? "Andika jina la kanda" : "Type a zone name"}
          ...
          <div className="mt-4 space-y-3">
            {filteredZones.slice(0, 4).map((zone) => (
              <div key={zone.id} className="rounded-lg bg-gray-50 dark:bg-gray-700 p-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  {zone.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {zone.church_type_display || zone.church_type}
                </p>
              </div>
            ))}
            {zoneQuery && filteredZones.length === 0 && (
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw" ? "Hakuna kanda iliyopatikana" : "No zones found"}
              </p>
            )}
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                {language === "sw" ? "Tafuta Mikoa" : "Search Regions"}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw"
                  ? "Tafuta mikoa kwenye mfumo"
                  : "Search regions across the system"}
              </p>
            </div>
          </div>
          <input
            ...
            placeholder={language === "sw" ? "Andika jina la mkoa" : "Type a region name"}
            className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-4 py-2 text-gray-900 dark:text-white"
          />
          <div className="mt-4 space-y-3">
            {filteredRegions.slice(0, 4).map((region) => (
              <div key={region.id} className="rounded-lg bg-gray-50 dark:bg-gray-700 p-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  {region.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {region.church_type_display || region.church_type}
                </p>
              </div>
            ))}
            {regionQuery && filteredRegions.length === 0 && (
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw" ? "Hakuna mkoa uliopatikana" : "No regions found"}
              </p>
            )}
          </div>
          <div className="mt-4 text-right">
