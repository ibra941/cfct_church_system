...
const DistrictDashboard = () => {
  ...
  const handleExport = async (fileFormat) => {
    try {
      const scopeDistrictId = districtId || user?.church_id || user?.church?.id;
      const response = await api.get(
        `/reports/export/district/?file_format=${fileFormat}${scopeDistrictId ? `&district_id=${scopeDistrictId}` : ""}`,
        {
          responseType: "blob",
        },
      );
