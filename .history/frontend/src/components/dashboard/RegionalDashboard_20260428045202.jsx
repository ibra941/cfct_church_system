...
const RegionalDashboard = () => {
  ...
  const [transferStats, setTransferStats] = useState({ total: 0, pending: 0, approved: 0, rejected: 0 });
  const [regionalFinancial, setRegionalFinancial] = useState({ grand_total: 0, by_district: [], by_type: [] });
  const [loading, setLoading] = useState(true);

  // Modals / panels
  const [showBroadcastPanel, setShowBroadcastPanel] = useState(false);
  const [broadcastForm, setBroadcastForm] = useState({ title: "", message: "", target_role: "district_leader" });
