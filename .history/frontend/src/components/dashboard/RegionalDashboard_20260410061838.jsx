...
const RegionalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch district and local church data for search
      const districtsResponse = await api.get("/districts/");
      setDistrictData(districtsResponse.data || []);

      const localsResponse = await api.get("/locals/");
      setLocalChurches(localsResponse.data || []);
      ...
      setTopChurches(topChurchesResponse.data || []);
    } catch (error) {
      ...
      setDistrictData([
        {
          id: 1,
          name: language === "sw" ? "Wilaya ya Kati" : "Central District",
          churches: 0,
          members: 0,
          offerings: 0,
        },
        {
          id: 2,
          name: language === "sw" ? "Wilaya ya Kaskazini" : "Northern District",
          churches: 0,
          members: 0,
          offerings: 0,
        },
        {
          id: 3,
          name: language === "sw" ? "Wilaya ya Kusini" : "Southern District",
          churches: 0,
          members: 0,
          offerings: 0,
        },
  ...
  const filteredLocalChurches = localChurches.filter((church) =>
  ...
  return (
    <div>
      {/* Search and discovery for regional leadership */}
      <div className="grid gap-4 mb-6 lg:grid-cols-2">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              {language === "sw" ? "Tafuta Wilaya" : "Search Districts"}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "sw"
                ? "Tafuta wilaya ndani ya mkoa wako"
                : "Search districts in your region"}
            </p>
          </div>
          <input
            type="text"
            ...
            placeholder={language === "sw" ? "Andika jina la wilaya" : "Type a district name"}
          ...
          <div className="mt-4 space-y-3">
            {filteredDistricts.slice(0, 4).map((district) => (
              <div key={district.id} className="rounded-lg bg-gray-50 dark:bg-gray-700 p-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  {district.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {district.church_type_display || district.church_type}
                </p>
              </div>
            ))}
            {districtQuery && filteredDistricts.length === 0 && (
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw" ? "Hakuna wilaya iliyopatikana" : "No districts found"}
              </p>
            )}
          </div>
          <div className="mt-4 text-right">
            ...
            >
              {language === "sw" ? "Tazama Makadirio" : "View all churches"} →
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              {language === "sw" ? "Tafuta Makanisa" : "Search Local Churches"}
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {language === "sw"
                ? "Tafuta makanisa ndani ya mkoa wako"
                : "Search local churches in your region"}
            </p>
          </div>
          <input
            type="text"
            ...
            placeholder={language === "sw" ? "Andika jina la kanisa" : "Type a church name"}
            className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 px-4 py-2 text-gray-900 dark:text-white"
          />
          <div className="mt-4 space-y-3">
            {filteredLocalChurches.slice(0, 4).map((church) => (
              <div key={church.id} className="rounded-lg bg-gray-50 dark:bg-gray-700 p-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  {church.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {church.church_type_display || church.church_type}
                </p>
              </div>
            ))}
            {localQuery && filteredLocalChurches.length === 0 && (
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {language === "sw" ? "Hakuna kanisa lililopatikana" : "No churches found"}
              </p>
            )}
          </div>
          <div className="mt-4 text-right">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          {language === "sw" ? "Dashibodi ya Mkoa" : "Regional Dashboard"}
        </h1>
