...
const ZoneDashboard = () => {
  ...
  const [budgetPlan, setBudgetPlan] = useState({ regions: [], total_budget: 0 });
  ...
  const handleRegionalApproval = async (userId, action) => {
    try {
      ...
      await api.post(endpoint, action === "reject" ? { reason: "Not approved at zone level" } : {});
      toast.success(
        language === "sw"
  ...
  const handleAssignRegionalLeader = async () => {
    if (!assignRegionId || !assignUserId) {
      toast.error(language === "sw" ? "Chagua mkoa na mtumiaji" : "Select region and user");
      return;
    }
    try {
  ...
  const handleExport = async (format) => {
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", `zone-report.${format === "excel" ? "xlsx" : format}`);
  ...
  const handleResourceRequest = async (event) => {
    ...
    try {
      ...
      toast.success(language === "sw" ? "Ombi limetumwa" : "Resource request sent");
      setResourceForm({ title: "", message: "" });
    } catch (error) {
      toast.error(language === "sw" ? "Imeshindikana kutuma ombi" : "Failed to send request");
    }
  };
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-8 mt-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Ulinganisho wa Mikoa" : "Regional Performance Comparison"}
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b dark:border-gray-700">
                  <th className="py-2">{language === "sw" ? "Mkoa" : "Region"}</th>
                  <th className="py-2">{language === "sw" ? "Wilaya" : "Districts"}</th>
                  <th className="py-2">{language === "sw" ? "Wanachama" : "Members"}</th>
                  <th className="py-2">{language === "sw" ? "Matoleo" : "Offerings"}</th>
                </tr>
              </thead>
              <tbody>
                {regionComparison.map((row) => (
                  <tr key={row.region_id} className="border-b dark:border-gray-700">
                    <td className="py-2">{row.region_name}</td>
                    <td className="py-2">{row.districts_count}</td>
                    <td className="py-2">{row.members_count}</td>
                    <td className="py-2">{formatCurrency(row.offerings_total || 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Muhtasari wa Fedha za Kanda" : "Zone Financial Summary"}
          </h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
            {language === "sw" ? "Jumla ya mapato" : "Grand total offerings"}: {formatCurrency(zoneFinancial.grand_total || 0)}
          </p>
          <div className="space-y-2">
            {(zoneFinancial.by_region || []).map((row) => (
              <div key={row.region_id} className="flex items-center justify-between text-sm">
                <span>{row.region_name}</span>
                <span className="font-semibold">{formatCurrency(row.total_offerings || 0)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8 mt-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Idhini za Viongozi wa Mikoa" : "Regional Leader Approvals"}
          </h2>
          <div className="space-y-3">
            {pendingRegionalApprovals.map((candidate) => (
              <div key={candidate.id} className="border dark:border-gray-700 rounded p-3">
                <p className="font-medium text-gray-900 dark:text-white">{candidate.full_name || candidate.username}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{candidate.email || "-"}</p>
                <div className="mt-2 flex gap-2">
                  <button
                    onClick={() => handleRegionalApproval(candidate.id, "approve")}
                  ...
                  <button
                    onClick={() => handleRegionalApproval(candidate.id, "reject")}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Mpe Mkoa Kiongozi" : "Assign Regional Leaders"}
          </h2>
          <div className="space-y-3">
            <select
              className="input"
            ...
            >
              <option value="">{language === "sw" ? "Chagua Mkoa" : "Select region"}</option>
              {regions.map((region) => (
            ...
            <button onClick={handleAssignRegionalLeader} className="btn-primary w-full">
              {language === "sw" ? "Mpe Mkoa Kiongozi" : "Assign Regional Leader"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 mt-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          <div className="space-y-2">
            <button onClick={() => handleExport("csv")} className="btn-secondary w-full flex items-center justify-center gap-2">
              <FaDownload /> CSV
            </button>
            <button onClick={() => handleExport("excel")} className="btn-secondary w-full flex items-center justify-center gap-2">
              <FaDownload /> Excel
            </button>
            <button onClick={() => handleExport("pdf")} className="btn-secondary w-full flex items-center justify-center gap-2">
              <FaDownload /> PDF
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Mawasiliano ya Kanda" : "Zone Communication Blast"}
          </h2>
          <form className="space-y-2" onSubmit={handleBroadcast}>
            ...
            <button className="btn-primary w-full" type="submit">
              ...
              {language === "sw" ? "Tuma kwa Viongozi wa Mikoa" : "Broadcast to Regional Leaders"}
            </button>
          </form>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
            {language === "sw" ? "Ombi la Rasilimali (Taifa)" : "Request National Resources"}
          </h2>
          <form className="space-y-2" onSubmit={handleResourceRequest}>
            ...
            <textarea
              ...
              onChange={(e) => setResourceForm((prev) => ({ ...prev, message: e.target.value }))}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mt-8">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">
          {language === "sw" ? "Mpango wa Bajeti ya Kanda" : "Zone Budget Planning & Allocation"}
        </h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b dark:border-gray-700">
                <th className="py-2">{language === "sw" ? "Mkoa" : "Region"}</th>
                <th className="py-2">{language === "sw" ? "Uwiano wa Wanachama" : "Member Share"}</th>
                <th className="py-2">{language === "sw" ? "Uwiano wa Matoleo" : "Offering Share"}</th>
                <th className="py-2">{language === "sw" ? "Mgawo Pendekezwa" : "Recommended Allocation"}</th>
              </tr>
            </thead>
            <tbody>
              {(budgetPlan.regions || []).map((row) => (
                <tr key={row.region_id} className="border-b dark:border-gray-700">
                  ...
                  <td className="py-2">{formatCurrency(row.recommended_allocation || 0)}</td>
                </tr>
              ))}
            </tbody>
          </table>
