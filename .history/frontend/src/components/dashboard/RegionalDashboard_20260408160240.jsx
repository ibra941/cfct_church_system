...
const RegionalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        monthlyGrowth: statsResponse.data.monthly_growth || 10,
        attendanceRate: statsResponse.data.attendance_rate || 75,
      });

      // Fetch district data - FIXED: handle paginated response
      ...
      setTopChurches([
        { id: 1, name: "CFCT Kati", offerings: 850000, members: 150 },
        { id: 2, name: "CFCT Kaskazini", offerings: 620000, members: 95 },
        { id: 3, name: "CFCT Kusini", offerings: 580000, members: 88 },
      ]);
      setRecentMembers([]);
      setRecentEvents([]);
    } finally {
  ...
  return (
    <div>
      {/* Navigation Buttons - Regional can go down to District and Local */}
      <div className="mb-6 flex items-center justify-between">
        <div className="flex space-x-3">
          <Link
            to="/dashboard/district"
            className="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition"
          >
            <FaArrowRight />
            <span>{language === "sw" ? "Kwa Wilaya" : "To Districts"}</span>
          </Link>
          <Link
            to="/dashboard/zone"
            className="bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600 px-4 py-2 rounded-lg flex items-center space-x-2 transition"
          >
            <FaArrowLeft className="mr-2" />
            <span>{language === "sw" ? "Kwa Kanda" : "To Zone"}</span>
          </Link>
        </div>
        <div className="text-sm text-gray-500 dark:text-gray-400">
          ...
          {" → "}
          <span className="text-gray-400">Wilaya → Kanisa</span>
        </div>
      </div>
...
export default RegionalDashboard;s