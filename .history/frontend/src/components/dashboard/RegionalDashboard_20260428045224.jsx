...
const RegionalDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      try {
        const dlResponse = await api.get("/members/?role=district_leader&limit=20");
      ...
      try {
        ...
        setTransferStats(tsResponse.data || { total: 0, pending: 0, approved: 0, rejected: 0 });
      } catch (e) {
      ...
      try {
        ...
        setRegionalFinancial(rfResponse.data || { grand_total: 0, by_district: [], by_type: [] });
      } catch (e) {
  ...
  const handleBroadcastSend = async (e) => {
    ...
    try {
      ...
      setBroadcastMsg(language === "sw" ? "Ujumbe umetumwa." : "Message broadcast successfully.");
      setBroadcastForm({ title: "", message: "", target_role: "district_leader" });
    } catch (err) {
      setBroadcastMsg(language === "sw" ? "Hitilafu imetokea." : "Failed to send broadcast.");
    } finally {
  ...
  const handleAssignDistrictLeader = async (e) => {
    ...
    setAssignMsg("");
    try {
      const res = await api.post(`/churches/${assignTargetChurch.id}/assign-district-leader/`, { user_id: assignUserId });
      setAssignMsg(res.data.message || (language === "sw" ? "Imefanikiwa." : "Success."));
      setAssignUserId("");
    } catch (err) {
      setAssignMsg(err.response?.data?.error || (language === "sw" ? "Hitilafu imetokea." : "An error occurred."));
    } finally {
  ...
  const handleResourceRequest = async (e) => {
    ...
    try {
      ...
      setResourceMsg(language === "sw" ? "Ombi limetumwa." : "Resource request sent.");
      setResourceForm({ title: "", message: "" });
    } catch (err) {
      setResourceMsg(language === "sw" ? "Hitilafu imetokea." : "Failed to send request.");
    } finally {
  ...
  const handleExport = async (format) => {
    try {
      const resp = await api.get(`/reports/export/regional/?file_format=${format}`, { responseType: "blob" });
