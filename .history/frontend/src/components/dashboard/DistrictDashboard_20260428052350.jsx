...
const DistrictDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      try {
        ...
        const merged = [...(Array.isArray(leaders) ? leaders : []), ...(Array.isArray(members) ? members : [])];
  ...
  const handleAssignPastor = async (e) => {
    ...
    setAssignMsg("");
    try {
      const response = await api.post(`/churches/${assignChurch.id}/assign-pastor/`, {
        user_id: assignUserId,
      });
      setAssignMsg(response.data.message || (language === "sw" ? "Imefanikiwa" : "Success"));
      await fetchDashboardData();
    } catch (err) {
      setAssignMsg(
        err.response?.data?.error ||
          (language === "sw" ? "Imeshindikana kumteua mchungaji" : "Failed to assign pastor"),
      );
    } finally {
  ...
  const handleBroadcast = async (e) => {
    ...
    try {
      ...
      setBroadcastMsg(response.data.message || (language === "sw" ? "Ujumbe umetumwa" : "Broadcast sent"));
      setBroadcastForm({ title: "", message: "" });
    } catch (err) {
      setBroadcastMsg(
        err.response?.data?.error ||
          (language === "sw" ? "Imeshindikana kutuma ujumbe" : "Failed to send broadcast"),
      );
    } finally {
  ...
  const handleResourceRequest = async (e) => {
    ...
    try {
      ...
      setResourceMsg(response.data.message || (language === "sw" ? "Ombi limetumwa" : "Request sent"));
      setResourceForm({ title: "", message: "" });
    } catch (err) {
      setResourceMsg(
        err.response?.data?.error ||
          (language === "sw" ? "Imeshindikana kutuma ombi" : "Failed to send request"),
      );
    } finally {
  ...
  const handleExport = async (fileFormat) => {
    try {
      const response = await api.get(`/reports/export/district/?file_format=${fileFormat}`, {
        responseType: "blob",
      });
  ...
  return (
    <div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg text-center">
            <p className="text-xl font-bold text-gray-900 dark:text-white">{transferStats.total}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">{language === "sw" ? "Jumla" : "Total"}</p>
          </div>
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg text-center">
            <p className="text-xl font-bold text-yellow-600">{transferStats.pending}</p>
            <p className="text-xs text-yellow-700 dark:text-yellow-300">{language === "sw" ? "Inasubiri" : "Pending"}</p>
          </div>
          <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center">
            <p className="text-xl font-bold text-green-600">{transferStats.approved}</p>
            <p className="text-xs text-green-700 dark:text-green-300">{language === "sw" ? "Imekubaliwa" : "Approved"}</p>
          </div>
          <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg text-center">
            <p className="text-xl font-bold text-red-600">{transferStats.rejected}</p>
            <p className="text-xs text-red-700 dark:text-red-300">{language === "sw" ? "Imekataliwa" : "Rejected"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Ulinganisho wa Makanisa" : "Church Comparison Report"}
          </h2>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                <th className="px-4 py-2 text-left text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Kanisa" : "Church"}</th>
                <th className="px-4 py-2 text-left text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Mchungaji" : "Pastor"}</th>
                <th className="px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Wanachama" : "Members"}</th>
                <th className="px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Idara" : "Departments"}</th>
                <th className="px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Matukio" : "Events"}</th>
                <th className="px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Matoleo" : "Offerings"}</th>
                <th className="px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Yanayosubiri" : "Pending"}</th>
                <th className="px-4 py-2 text-left text-xs text-gray-500 dark:text-gray-300 uppercase">{language === "sw" ? "Kitendo" : "Action"}</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {churchComparison.map((church) => (
                <tr key={church.church_id}>
                  <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{church.church_name}</td>
                  <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-300">{church.pastor_name || "-"}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-500 dark:text-gray-300">{church.members_count}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-500 dark:text-gray-300">{church.departments_count}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-500 dark:text-gray-300">{church.events_count}</td>
                  <td className="px-4 py-3 text-sm text-right text-green-600">{formatCurrency(church.offerings_total)}</td>
                  <td className="px-4 py-3 text-sm text-right text-yellow-600">{church.pending_registrations}</td>
                  <td className="px-4 py-3 text-sm">
                    <button
                      onClick={() => {
                        const selected = localChurchData.find((c) => c.id === church.church_id);
                        setAssignChurch(selected || { id: church.church_id, name: church.church_name });
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-md font-bold text-gray-900 dark:text-white">
              {language === "sw" ? "Tuma Ujumbe kwa Wachungaji" : "Broadcast to Pastors"}
            </h3>
          ...
          <form onSubmit={handleBroadcast} className="space-y-3">
            ...
            >
              {broadcastLoading ? (language === "sw" ? "Inatuma..." : "Sending...") : (language === "sw" ? "Tuma" : "Send")}
            </button>
          </form>
          {broadcastMsg && <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{broadcastMsg}</p>}
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-md font-bold text-gray-900 dark:text-white">
              {language === "sw" ? "Omba Rasilimali kwa Mkoa" : "Request Resources from Region"}
            </h3>
          ...
          <form onSubmit={handleResourceRequest} className="space-y-3">
            <input
              ...
              placeholder={language === "sw" ? "Kichwa cha Ombi" : "Request Title"}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              required
            />
            <textarea
            ...
            >
              {resourceLoading ? (language === "sw" ? "Inatuma..." : "Sending...") : (language === "sw" ? "Tuma Ombi" : "Send Request")}
            </button>
          </form>
          {resourceMsg && <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{resourceMsg}</p>}
        </div>
      </div>
      ...
      <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-md font-bold text-gray-900 dark:text-white">
            {language === "sw" ? "Pakua Ripoti za Wilaya" : "Export District Reports"}
          </h3>
              ...
              >
                ...
                {pastorCandidates.map((candidate) => (
                  <option key={candidate.id} value={candidate.id}>
                    {(candidate.full_name || candidate.username) + " - " + (candidate.church_name || "")}
                  </option>
                ))}
              </select>
              <button
                ...
                className="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm hover:bg-primary-700 disabled:opacity-50"
              >
                {assignLoading ? (language === "sw" ? "Inahifadhi..." : "Saving...") : (language === "sw" ? "Teua" : "Assign")}
              </button>
            </form>
            {assignMsg && <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{assignMsg}</p>}
          </div>
        </div>
      )}
