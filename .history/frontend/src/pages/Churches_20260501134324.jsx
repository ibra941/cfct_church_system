...
const Churches = () => {
  ...
  const availableDistricts = useMemo(() => {
    if (filters.regionId) {
      return districts.filter(
        (district) => String(district.parent_church) === String(filters.regionId),
      );
    }
    return districts;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.parent_church) {
      toast.error(
        language === "sw"
          ? "Tafadhali chagua wilaya ya kanisa kwanza."
          : "Please select the church district first.",
      );
      return;
    }
    try {
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {churches.map((church) => (
                  <tr key={church.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                      {church.parent_church_name || church.city || church.region || "-"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <button
                        onClick={() => {
                          ...
                          setFormData({
                            ...church,
                            church_type: church.church_type || "local",
                            parent_church: church.parent_church || "",
                          });
                          setShowModal(true);
                        }}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 max-h-screen overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    {language === "sw" ? "Aina ya Kituo" : "Church Type"}
                  </label>
                  <input
                    type="text"
                    className="input bg-gray-100 dark:bg-gray-700"
                    value={language === "sw" ? "Kanisa la Mtaa" : "Local Church"}
                    disabled
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    {language === "sw" ? "Wilaya ya Kanisa" : "Church District"}
                  </label>
                  <select
                    required
                    className="input"
                    value={formData.parent_church}
                    onChange={(e) =>
                      setFormData({ ...formData, parent_church: e.target.value })
                    }
                  >
                    <option value="">
