...
const ManageUsers = () => {
  ...
  const handleDeleteUser = async (userItem) => {
    ...
    try {
      ...
      toast.success(
        language === "sw" ? "Mtumiaji amefutwa" : "User deleted",
      );
                ...
                {users.map((userItem) => (
                  <tr key={userItem.id}>
                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">@{userItem.username}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                      <div>{userItem.email || "-"}</div>
                      <div className="text-xs text-gray-400 dark:text-gray-500">{userItem.phone || "-"}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
