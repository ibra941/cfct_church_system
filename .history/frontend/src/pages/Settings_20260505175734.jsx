...
const Settings = () => {
  ...
  const updateContact = async (e) => {
    ...
    try {
      const payload = {
        ...
        neighborhood: contactForm.neighborhood?.trim() || "",
      };
  ...
  const updateProfilePicture = async (e) => {
    ...
    if (!ALLOWED_TYPES.includes(profilePicture.type)) {
      toast.error(language === "sw" ? "Aina ya faili haifai. Tumia JPEG, PNG, au WebP." : "Invalid file type. Use JPEG, PNG, or WebP.");
      return;
    }
    if (profilePicture.size > MAX_SIZE_MB * 1024 * 1024) {
      toast.error(language === "sw" ? `Picha kubwa mno. Ukubwa wa juu ni ${MAX_SIZE_MB}MB.` : `File too large. Maximum size is ${MAX_SIZE_MB}MB.`);
      return;
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          <div className="flex justify-between items-center">
            <span className="text-gray-700 dark:text-gray-300">
              {language === "sw" ? "Wezesha Arifa" : "Enable Notifications"}
            </span>
            <button
              type="button"
              onClick={() => {
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${notificationsEnabled ? "bg-primary-600" : "bg-gray-300"}`}
            >
              <span
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
            {language === "sw"
              ? "Zima arifa ili usipate ujumbe wa mbele."
              : "Disable to stop receiving in-app notifications."}
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          <form onSubmit={updateContact} className="space-y-3">
            ...
            <input
              className="input"
              placeholder={
                language === "sw" ? "Mtaa / Eneo" : "Neighborhood / Area"
              }
