...
const RegisterApproval = () => {
  ...
  const fetchDuplicates = async () => {
    ...
    try {
      ...
      console.error(error);
      setDuplicateData({ duplicate_email_groups: [], duplicate_phone_groups: [] });
      toast.error(
        language === "sw"
  ...
  const handleCsvImport = async () => {
    if (!csvFile) {
      toast.error(
        language === "sw" ? "Chagua faili la CSV kwanza" : "Select a CSV file first",
      );
      return;
    }
    ...
    try {
      ...
      const response = await api.post("/members/registrations/import/csv/", formData, {
      ...
      console.error(error);
      toast.error(
        language === "sw" ? "Uingizaji wa CSV umeshindikana" : "CSV import failed",
      );
    } finally {
  ...
  const renderDuplicateGroup = (group, type) => {
    ...
    const selectedPrimary =
      selectedPrimaryByGroup[groupKey] || members[0]?.id;
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-3">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            <span className="font-semibold">
              {type === "email" ? "Email" : language === "sw" ? "Simu" : "Phone"}:
            </span>{" "}
            {groupKeyValue}
          </p>
          <div className="flex items-center gap-2">
            <label className="text-xs text-gray-500 dark:text-gray-400">
              {language === "sw" ? "Hifadhi kama rekodi kuu" : "Keep as primary"}
            </label>
            <select
            ...
            return (
              ...
              >
                ...
                {isPrimary ? (
                  ...
                  >
                    {language === "sw" ? "Unganisha kwenye rekodi kuu" : "Merge into primary"}
                  </button>
                )}
              </div>
            );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
        ...
        {csvImportResult && (
          <div className="mt-3 text-sm text-gray-700 dark:text-gray-300">
            <p>
              {language === "sw" ? "Zilizoundwa" : "Created"}: {csvImportResult.created_count || 0}
              {" | "}
              {language === "sw" ? "Zilirukwa" : "Skipped"}: {csvImportResult.skipped_count || 0}
            </p>
            {(csvImportResult.errors || []).length > 0 && (
              <div className="mt-2">
                <p className="font-medium text-red-600 dark:text-red-400">
                  {language === "sw" ? "Makosa ya uingizaji:" : "Import errors:"}
                </p>
                <ul className="list-disc ml-5 mt-1 space-y-1">
                  {(csvImportResult.errors || []).slice(0, 5).map((err, index) => (
                    <li key={`${err.row}-${index}`}>Row {err.row}: {err.error}</li>
                  ))}
                </ul>
              </div>
            )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mt-6">
        ...
        {duplicatesLoading ? (
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {language === "sw" ? "Inatafuta marudio..." : "Loading duplicates..."}
          </p>
        ) : (
          <div className="space-y-4">
            <div>
              ...
              {(duplicateData.duplicate_email_groups || []).length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {language === "sw" ? "Hakuna marudio ya email" : "No duplicate emails found"}
                </p>
              ) : (
            ...
            <div>
              ...
              {(duplicateData.duplicate_phone_groups || []).length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {language === "sw" ? "Hakuna marudio ya simu" : "No duplicate phones found"}
                </p>
              ) : (
                <div className="space-y-3">
