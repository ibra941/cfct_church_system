...
const AddLeader = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.preferred_church_id) {
      toast.error(language === "sw" ? "Chagua kanisa" : "Please select a church");
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.error(
        backendError ||
          (language === "sw" ? "Hitilafu imetokea. Jaribu tena." : "An error occurred. Please try again.")
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="max-w-2xl bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {language === "sw" ? "Jukumu na Taarifa Binafsi" : "Role & Personal Information"}
              </h3>
              ...
              <input
                type="text"
                placeholder={language === "sw" ? "Mtaa / Kitongoji" : "Neighborhood"}
                className="input"
                value={formData.personal_info.neighborhood}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    personal_info: { ...prev.personal_info, neighborhood: e.target.value },
                  }))
                }
              />
              ...
              <button
                ...
                onClick={() => {
                  if (!formData.role) {
                    toast.error(language === "sw" ? "Chagua jukumu kwanza" : "Please select a role first");
                    return;
                  }
                  if (!formData.personal_info.full_name || !formData.personal_info.phone) {
                    toast.error(language === "sw" ? "Jina na simu vinahitajika" : "Full name and phone are required");
                    return;
                  }
                  setStep(2);
                }}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {language === "sw" ? "Taarifa za Ziada (Hiari)" : "Additional Info (Optional)"}
              </h3>
              <input
                type="text"
                placeholder={language === "sw" ? "Jina la Mlezi / Ndugu" : "Guardian / Next of Kin Name"}
                className="input"
                value={formData.guardian_info.guardian_name}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    guardian_info: { ...prev.guardian_info, guardian_name: e.target.value },
                  }))
                }
              />
              <input
                type="tel"
                placeholder={language === "sw" ? "Simu ya Mlezi" : "Guardian Phone"}
                className="input"
                value={formData.guardian_info.guardian_phone}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    guardian_info: { ...prev.guardian_info, guardian_phone: e.target.value },
                  }))
                }
              />
              <input
                type="text"
                placeholder={language === "sw" ? "Uhusiano na Mlezi" : "Relationship to Guardian"}
                className="input"
                value={formData.guardian_info.relationship}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    guardian_info: { ...prev.guardian_info, relationship: e.target.value },
                  }))
                }
              />
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {language === "sw" ? "Kanisa la Mahali na Taarifa za Kiroho" : "Local Church & Spiritual Info"}
              </h3>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      spiritual_info: { ...prev.spiritual_info, date_of_birth: e.target.value },
                    }))
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      spiritual_info: { ...prev.spiritual_info, christian_birth_date: e.target.value },
                    }))
                  }
                />
              </div>
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="mb-3">
                    ...
                    {regions.length === 0 && (
                      <p className="text-xs text-gray-400 mt-1">
                        {language === "sw" ? "Hakuna mikoa katika kanda hii" : "No regions found for this zone"}
                      </p>
                    )}
                  </div>
                )}
                ...
                {selectedRegion && (
                  <div className="mb-3">
                    ...
                    {districts.length === 0 && (
                      <p className="text-xs text-gray-400 mt-1">
                        {language === "sw" ? "Hakuna wilaya katika mkoa huu" : "No districts found for this region"}
                      </p>
                    )}
                  </div>
                )}
                ...
                {selectedDistrict && (
                  <div className="mb-3">
                    ...
                    {localChurches.length === 0 && (
                      <p className="text-xs text-gray-400 mt-1">
                        {language === "sw" ? "Hakuna makanisa katika wilaya hii" : "No churches found for this district"}
                      </p>
                    )}
                  </div>
                )}
