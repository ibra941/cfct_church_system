  ...
  const openAddModal = (tabId) => {
    ...
    if (tabId === "homepage") {
      ...
      const availableSections = SECTION_OPTIONS.filter(
        (section) => !usedSections.has(section),
      );
      if (availableSections.length === 0) {
        toast.error(
          "All homepage sections already exist. Edit existing sections instead.",
        );
        return;
      }
      setFormData({
            ...
            <div className="flex justify-end space-x-3 mt-4">
              ...
              <button
                onClick={async () => {
                  ...
                  if (editingItem) {
                    await handleSave(editingItem, formData, endpoint);
                  } else {
                    await handleAdd(formData, endpoint);
                  }
                }}
