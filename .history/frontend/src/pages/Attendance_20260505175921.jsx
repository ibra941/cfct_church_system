...
const Attendance = () => {
  ...
  const fetchMyHistory = async () => {
    ...
    try {
      ...
      const response = await api.get(`/attendance/my-history/${query}`);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
          {language === "sw" ? "Chuja Historia" : "Filter History"}
        </h2>
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
              {language === "sw" ? "Tarehe ya Mwanzo" : "Start Date"}
            </label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
              {language === "sw" ? "Tarehe ya Mwisho" : "End Date"}
            </label>
            <input
              type="date"
              className="input text-sm py-1"
          ...
          <div>
            ...
            >
              ...
              <option value="sunday_service">{language === "sw" ? "Ibada ya Jumapili" : "Sunday Service"}</option>
              <option value="midweek">{language === "sw" ? "Ibada ya Kati ya Wiki" : "Midweek Service"}</option>
              <option value="prayer_meeting">{language === "sw" ? "Mkutano wa Maombi" : "Prayer Meeting"}</option>
              <option value="bible_study">{language === "sw" ? "Masomo ya Biblia" : "Bible Study"}</option>
              <option value="special_event">{language === "sw" ? "Tukio Maalum" : "Special Event"}</option>
              <option value="small_group">{language === "sw" ? "Kikundi Kidogo" : "Small Group"}</option>
            </select>
          </div>
          <button
            type="button"
          ...
          {(startDate || endDate || serviceTypeFilter) && (
            <button
              ...
              onClick={() => { setStartDate(""); setEndDate(""); setServiceTypeFilter(""); }}
            >
