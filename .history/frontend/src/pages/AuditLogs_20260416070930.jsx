    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input
            ...
            placeholder={language === "sw" ? "Tafuta mtumiaji/jedwali/kitu" : "Search user/table/object"}
          />
          <select
            value={actionFilter}
            ...
            className="border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm bg-white dark:bg-gray-900"
          >
            <option value="">{language === "sw" ? "Vitendo vyote" : "All actions"}</option>
      ...
      {loading ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 text-sm text-gray-500 dark:text-gray-400">
          {language === "sw" ? "Inapakia rekodi za ukaguzi..." : "Loading audit logs..."}
        </div>
      ) : errorMessage ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 text-sm text-gray-500 dark:text-gray-400">
          {language === "sw" ? "Hakuna rekodi zilizopatikana." : "No logs found."}
        </div>
      ) : (
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {logs.map((log) => (
                <tr key={log.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300 max-w-sm truncate" title={log.object_repr}>
                    {log.object_repr || "-"}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
