...
const SermonArchive = () => {
  ...
  const handleSave = async () => {
    if (!form.title.trim() || !form.sermon_date) {
      toast.error(t("Kichwa na tarehe vinahitajika", "Title and date are required"));
      return;
    }
    setSaving(true);
    try {
      ...
      toast.error(t("Imeshindikana kuhifadhi", "Failed to save"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("Una uhakika wa kufuta mahubiri haya?", "Are you sure you want to delete this sermon?")))
      return;
    try {
      ...
      toast.error(t("Imeshindikana kufuta", "Failed to delete"));
    }
  };

  // Unique series for filter chips
  const allSeries = [...new Set(sermons.map((s) => s.series_name).filter(Boolean))];
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-primary-200 dark:border-primary-700">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900 dark:text-white">
              {editingSermon ? t("Hariri Mahubiri", "Edit Sermon") : t("Mahubiri Mapya", "New Sermon")}
            </h2>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="date" className="input" value={form.sermon_date} onChange={(e) => setForm({ ...form, sermon_date: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                {t("Mstari wa Maandiko", "Scripture Reference")}
              </label>
              <input className="input" placeholder="e.g. John 3:16" value={form.scripture_reference} onChange={(e) => setForm({ ...form, scripture_reference: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
            ...
            <div>
              ...
              <input type="url" className="input" placeholder="https://..." value={form.audio_url} onChange={(e) => setForm({ ...form, audio_url: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                {t("Kiungo cha Video (URL)", "Video URL")}
              </label>
              <input type="url" className="input" placeholder="https://..." value={form.video_url} onChange={(e) => setForm({ ...form, video_url: e.target.value })} />
            </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            {isLeader
              ? t("Hakuna mahubiri bado. Bofya 'Ongeza Mahubiri' kuanza.", "No sermons yet. Click 'Add Sermon' to get started.")
              : t("Hakuna mahubiri yaliyohifadhiwa bado.", "No sermons have been archived yet.")}
          </p>
        </div>
      ) : (
