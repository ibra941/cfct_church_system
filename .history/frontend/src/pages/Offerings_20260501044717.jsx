...
const Offerings = () => {
  ...
  const canManageChurchPaymentSettings = isLeader && !!user?.church;
  ...
  const savePaymentSettings = async (e) => {
    ...
    try {
      ...
      setPaymentSettings(res.data);
      toast.success(
        T(language, "Namba za malipo zimehifadhiwa.", "Payment details saved."),
      );
    } catch (err) {
  ...
  const buildSummary = (list) => {
    ...
    list.forEach((o) => {
      map[o.offering_type] =
        (map[o.offering_type] || 0) + parseFloat(o.amount || 0);
    });
    setSummary(map);
  };
  ...
  const fetchBankDetails = async () => {
    try {
      ...
      toast.error(
        T(
          language,
          "Hitilafu kupata maelezo ya benki.",
  ...
  const handleCashSubmit = async (e) => {
    ...
    try {
      ...
      fetchOfferings();
      toast.success(
        T(language, "Matoleo yamehifadhiwa!", "Offering recorded!"),
      );
    } catch (err) {
      toast.error(extractError(err).substring(0, 150));
    } finally {
  ...
  const handleMobileMoneySubmit = async (e) => {
    ...
    if (!form.phone) {
      toast.error(
        T(language, "Ingiza nambari ya simu.", "Enter phone number."),
      );
      return;
    }
    setSubmitting(true);
  ...
  const handleBankTransferConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        T(
          language,
          "Matoleo yamerekodiwa. Inasubiri uthibitisho.",
          "Offering recorded. Awaiting verification.",
        ),
      );
    } catch (err) {
      toast.error(extractError(err).substring(0, 150));
  ...
  const paymentMethodLabel = (m) => {
    const map = {
      cash: T(language, "Fedha Taslimu", "Cash"),
      mobile_money: T(language, "Simu Pesa", "Mobile Money"),
      bank_transfer: T(language, "Benki", "Bank Transfer"),
    };
    return map[m] || m;
  };

  const selectedProviderNumber =
    paymentSettings?.mobile_money_numbers?.[form.operator]?.lipa_number || "";
    ...
    if (ps === "completed")
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
          <FaCheckCircle />
          {T(language, "Imekamilika", "Completed")}
        </span>
      );
    if (ps === "failed")
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300">
          <FaTimesCircle />
          {T(language, "Imeshindwa", "Failed")}
        </span>
      );
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300">
        <FaClock />
        {T(language, "Inasubiri", "Pending")}
      </span>
    );
  };
    ...
    <div>
      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-800 rounded-lg shadow-lg p-6 mb-6">
        <h3 className="text-white text-lg mb-2">
          {T(language, "Jumla ya Matoleo", "Total Offerings")}
        </h3>
        <p className="text-white text-4xl font-bold">
          {formatTZS(totalOfferings)}
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {T(language, "Muhtasari wa Matoleo", "Offerings Summary")}
            </h2>
          </div>
      ...
      {canManageChurchPaymentSettings && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {T(
                  language,
                  "Hariri Lipa Namba za simu pesa na akaunti ya benki kwa kanisa lako.",
                  "Update mobile money Lipa numbers and bank account details for your church.",
                )}
              </p>
            </div>
            {paymentSettings?.church_name && (
              <span className="text-xs px-3 py-1 rounded-full bg-primary-50 text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">
                {paymentSettings.church_name}
              </span>
            )}
          </div>
          ...
          <form onSubmit={savePaymentSettings} className="space-y-5">
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-3">
                {T(language, "Simu Pesa", "Mobile Money")}
              </h3>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-3">
                <input
                  ...
                  placeholder={T(language, "Jina la Benki", "Bank Name")}
                />
                <input
                  className="input"
                ...
                <input
                  ...
                  placeholder={T(language, "Jina la Akaunti", "Account Name")}
                />
                <input
                  className="input"
                  ...
                  placeholder={T(language, "Tawi la Benki", "Bank Branch")}
                />
                <input
                  className="input md:col-span-2"
                  value={settingsForm.bank_swift_code}
                  onChange={(e) =>
                    setSettingsForm((prev) => ({
                      ...prev,
            ...
            >
              {settingsSaving
                ? T(language, "Inahifadhi...", "Saving...")
                : T(
                    language,
                    "Hifadhi Namba za Malipo",
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          {step === "success" && (
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                {T(language, "Imekamilika!", "Done!")}
              </h3>
              <p className="text-sm text-gray-500 mb-4">
                {paymentMethod === "bank_transfer"
                  ? T(
                      language,
                      "Inasubiri uthibitisho wa benki.",
              ...
              {paymentResult && (
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-left text-sm space-y-1 mb-5">
                  {paymentResult.receipt_no && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">
                        {T(language, "Nambari ya Risiti", "Receipt No")}
                      </span>
                  ...
                  {paymentResult.transaction_reference && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">
                        {T(language, "Kumbukumbu", "Reference")}
                      </span>
                  ...
                  {paymentResult.payment_status && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-500">
                        {T(language, "Hali", "Status")}
                      </span>
                      {statusBadge(paymentResult.payment_status)}
                    </div>
                  )}
                </div>
              )}
              <button onClick={resetForm} className="btn-primary w-full">
                {T(language, "Rekodi Mengine", "Record Another")}
              </button>
            </div>
          )}
          ...
          {step === "failed" && (
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                {T(language, "Malipo Yameshindwa", "Payment Failed")}
              </h3>
              <p className="text-sm text-gray-500 mb-4">
                {T(
                  language,
              ...
              {paymentResult?.transaction_reference && (
                <p className="text-xs text-gray-400 mb-4 font-mono">
                  Ref: {paymentResult.transaction_reference}
                </p>
              )}
              <button onClick={resetForm} className="btn-primary w-full">
                {T(language, "Jaribu Tena", "Try Again")}
              </button>
            </div>
          )}
          ...
          {step === "pending_mm" && (
            <div className="text-center py-6">
              <div className="animate-pulse">
                <FaMobileAlt className="text-primary-600 text-5xl mx-auto mb-4" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                {T(
                  language,
              ...
              <p className="text-sm text-gray-500 mb-2">
                {T(language, "Ombi limetumwa kwa ", "Request sent to ")}{" "}
                <strong>{paymentResult?.operator_label}</strong>.
              </p>
              <p className="text-sm text-gray-400 mb-5">
                {T(
                  language,
                  "Ingiza PIN yako ya simu pesa kukamilisha malipo.",
              ...
              {paymentResult?.provider_number && (
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 mb-4 text-sm">
                  <p className="text-gray-500 mb-1">
                    {T(language, "Lipa Namba ya Kanisa", "Church Lipa Number")}
                  </p>
          ...
          {step === "bank_details" && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-4">
                {T(language, "Hamisha ", "Transfer ")}{" "}
                <strong>{formatTZS(form.amount)}</strong>{" "}
                {T(
                  language,
                  " kwa akaunti hii ukitumia nambari ya kumbukumbu.",
                  " to this account using the reference number.",
                )}
              </p>
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-4 space-y-2 text-sm">
                {[
                  ["Benki / Bank", bankDetails?.bank_name, false],
                  [
                    T(language, "Jina la Akaunti", "Account Name"),
                    bankDetails?.account_name,
                    false,
                  ],
                  [
                    T(language, "Nambari ya Akaunti", "Account Number"),
                  ...
                  ...(bankDetails?.swift_code
                    ? [["SWIFT", bankDetails.swift_code, true]]
                    : []),
                  ...
                  >
                    ...
                    <span
                      className={`font-semibold text-gray-900 dark:text-white ${mono ? "font-mono" : ""} ${!value ? "text-gray-400 italic text-xs" : ""}`}
                    >
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-xl p-4 mb-5">
                <p className="text-xs font-semibold text-yellow-700 dark:text-yellow-300 mb-1 uppercase tracking-wide">
                  ⚠{" "}
                  {T(
                    language,
                    "Tumia Nambari Hii ya Kumbukumbu",
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {T(
                    language,
                    "Lazima utumie nambari hii ili matoleo yako yahakikishwe.",
                    "You must use this reference so your offering can be verified.",
                  )}
                </p>
              </div>
                ...
                >
                  {submitting ? (
                    <span className="flex items-center justify-center gap-2">
                      ...
                      {T(language, "Inawasilisha...", "Submitting...")}
                    </span>
                  ) : (
                    T(
                      language,
                ...
                onSubmit={
                  paymentMethod === "cash"
                    ...
                    : paymentMethod === "mobile_money"
                      ...
                      : (e) => {
                          e.preventDefault();
                          if (validateBase()) fetchBankDetails();
                        }
                }
                className="space-y-4"
              >
                ...
                <input
                  ...
                  min="1"
                  step="any"
                  placeholder={T(language, "Kiasi (TZS)", "Amount (TZS)")}
                  className="input"
                  value={form.amount}
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, offering_type: e.target.value })
                  }
                >
                  {OFFERING_TYPES.map((o) => (
                    <option key={o.value} value={o.value}>
                      {language === "sw" ? o.sw : o.en}
                    </option>
                  ))}
                </select>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {OPERATORS.map((op) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setForm({ ...form, operator: op.value })
                            }
                            className={`py-2 px-3 rounded-lg border-2 text-sm font-medium transition-colors text-left ${form.operator === op.value ? `border-primary-600 bg-primary-50 dark:bg-primary-900/20 ${op.color}` : "border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400"}`}
                          >
                            {op.label}
                          </button>
                        ))}
                ...
                {paymentMethod === "bank_transfer" && (
                  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm text-blue-800 dark:text-blue-300">
                    ...
                    {T(
                      language,
                      "Bonyeza kitufe kupata nambari ya kumbukumbu na maelezo ya akaunti ya benki.",
                      "Click the button below to get a unique reference and bank account details.",
                    )}
                ...
                >
                  {submitting ? (
                    <span className="flex items-center justify-center gap-2">
                      <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" />
                      {T(language, "Tafadhali Subiri...", "Please Wait...")}
                    </span>
                  ) : paymentMethod === "cash" ? (
                    T(language, "Rekodi Matoleo", "Record Offering")
                  ) : paymentMethod === "mobile_money" ? (
                    T(language, "Lipa Sasa (STK Push)", "Pay Now (STK Push)")
                  ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">
            {T(language, "Matoleo ya Hivi Karibuni", "Recent Offerings")}
          </h2>
          <div className="space-y-3 max-h-[520px] overflow-y-auto pr-1">
            {offerings.length === 0 && (
              <p className="text-gray-500 text-center py-8">
                {T(language, "Hakuna matoleo bado.", "No offerings yet.")}
              </p>
            )}
            {offerings.slice(0, 20).map((o) => (
              <div
                key={o.id}
                className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="ml-2 text-xs text-gray-500">
                      {offeringTypeLabel(o.offering_type)}
                    </span>
                  </div>
